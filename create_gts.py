...
for index, text in enumerate(natsorted(glob(abs_path("dataset/val/val_txt/*")))):
    ...
    os.makedirs(abs_path(save_dir)) if os.path.isdir(
        abs_path(save_dir)
    ) is False else None
    ...
    for i in range(12):
        ...
        for i2 in temp:
            ...
            img[int(xmin) : int(xmax), int(ymin) : int(ymax)] = np.maximum(
                lm_tmp, img[int(xmin) : int(xmax), int(ymin) : int(ymax)]
            )
...
for index, text in enumerate(natsorted(glob(abs_path("dataset/train/train_txt/*")))):
    m = int(text.split("video")[1].split(".txt")[0])
    save_dir = abs_path("dataset/train/train_gts") + "/sequence{}".format(
        str(m).zfill(3)
    )
    os.makedirs(abs_path(save_dir)) if os.path.isdir(
        abs_path(save_dir)
