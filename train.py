...
def train_net(net, device, cfg, val_clip):
    ...
    train_loader = DataLoader(
        train,
        batch_size=cfg.train.video_number,
        worker_init_fn=worker_init_fn(cfg.train.seed),
    ...
    writer = SummaryWriter(
        log_dir=abs_path("./logs"), comment=f"LR_{cfg.train.lr}_BS_{batch_size}"
    )
    global_step = 0
        ...
        with tqdm(
            ...
            for batch in train_loader:
                ...
                if global_step % int(n_train // imgs.shape[0]) == 0:
                    val_loss = eval_net(
                        net, val_loader, device, n_val, val_clip, epoch, cfg
                    )
...
def main(cfg):
    ...
    os.makedirs(abs_path(cfg.output.dir)) if os.path.isdir(
        abs_path(cfg.output.dir)
    ) is False else None
    ...
    net = model(
        n_channels=3, n_classes=1, num=8, sub_batch=cfg.train.batch_size, bilinear=True
    )
    ...
    if cfg.load:
        net.load_state_dict(torch.load(cfg.load, map_location=device))
