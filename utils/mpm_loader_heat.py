...
class MPM_Dataset(Dataset):
    def __init__(self, cfg_type, cfg_data):
        ...
        for i, seq in enumerate(seqs):
            ...
            self.img_paths.extend([[path, seq_name] for path in sorted(glob(join(seq, '*')))][:-1])
            num_seq = len(listdir(seq)) - 1
            self.ids.extend([[i*temp_itv, num_seq - j] for j in range(num_seq)[:-temp_itv]])
    ...
    def __getitem__(self, i):
        ...
        for i in range(len(img_names)):
            ...
            mpm_file.append(glob(join(self.gts_dir, idx[1], f'{itv:03}', name) + '.*')[0])
        ...
        for id, x in enumerate(img_names):
            ...
            mag.append(np.expand_dims(cv2.cvtColor(np.load(mpm_file[id])['arr_0'], cv2.COLOR_BGR2GRAY), axis=0))
        ...
        if self.train==True:
            crop_factor = 0.5
            crop_size = (int(img.shape[1]*crop_factor),int(img.shape[2]*crop_factor))
            dx = int(random.randint(0,1)*img.shape[1]*1./2)
            dy = int(random.randint(0,1)*img.shape[0]*1./2)
